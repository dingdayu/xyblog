...
function doDel(id){
	...
	if (window.confirm('确实要永久删除选择项吗？'))
	{
		$.post(URL+"/foreverdelete/","id="+keyValue,function(data) {
			if (data.status == 1){
...
//分组添加
function groAdd(){
	location.href  = URL+"/groAdd/";
}
//管理员添加
function adminAdd(){
		 location.href  = URL+"/adminAdd/";
}
function adminEdit(id){
	...
	location.href =  URL+"/adminEdit/id/"+keyValue;
}
function groEdit(id){
	var keyValue;
	if (id)
	...
	location.href =  URL+"/groEdit/id/"+keyValue;
}
function groDel(id){
	var keyValue;
	if (id)
	...
	if (window.confirm('确实要永久删除选择项吗？'))
	{
		$.post(URL+"/groDel/","id="+keyValue,function(data) {
			if (data.status == 1){
				location.href  = ACTION;
			}
		});
...
function userDel(id){
	...
	if (window.confirm('确实要永久删除选择项吗？'))
	{
		$.post(URL+"/userDel/","id="+keyValue,function(data) {
			if (data.status == 1){
				location.href  = ACTION;
			}
		});
