...
class ComProjectAction extends BaseAction {
    ...
    public function index() {
		...
		$Comment = D('Comment');
		$count = $Comment->where('type=3')->count();// 查询满足要求的总记录数
		import('ORG.Util.Page', LIB_PATH);
		$Page       = new Page($count,10);// 实例化分页类 传入总记录数和每页显示的记录数
		$show       = $Page->show();// 分页显示输出
		// 进行分页数据查询 注意limit方法的参数要使用Page类的属性
		$list = $Comment->where('type=3')->order('createTime')->limit($Page->firstRow.','.$Page->listRows)->select();
