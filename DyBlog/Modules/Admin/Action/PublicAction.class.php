...
class PublicAction extends Action {
    ...
    public function top() {
        ...
        $model	=	M("Menu");
        $list	=	$model->where('type =1 and status=1')->getField('id,title');
		//dump($list);
    ...
    public function logout() {
        if(isset($_SESSION[C('USER_AUTH_KEY')])) {
            ...
            unset($_SESSION);
            session_destroy();
            $this->success('登出成功！',__APP__.'/Home/login/');
        }else {
            $this->error('已经登出！');
        }
    }
	...
	public function getInfo() {
        if(!isset($_SESSION[C('USER_AUTH_KEY')])) {
			...
			echo "document.write('<p class=\"think-user\">[ <a href=\"/Admin/\">".$_SESSION['loginUserName']."</a><a href=\"/Admin/Public/logout/\">退出</a> ]</p>');";
		}
    }
}