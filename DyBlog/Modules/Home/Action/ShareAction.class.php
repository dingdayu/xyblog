...
class ShareAction extends CommonAction {
	...
	public function index(){
		...
		$blogcount1      = $Blogs->where('status=1 and type=2')->count();// 查询满足要求的总记录数
		...
		$list = $Blogs->where($whe)->order('isTop desc, createTime desc')->limit($Page->firstRow.','.$Page->listRows)->getField('id,title,createTime,count,isTop,groupId,fileId,imgId,userId');
		
		foreach ($list as $k => $v) {
			$list[$k]['pcount'] = $Comment->where('status=1 and type=2 and fromId='.$v['id'])->count();
			$group = $Group->where('type=2 and id='.$v['groupId'])->getField('title');
			$list[$k]['group'] = (empty($group)) ? '未知分组' : $group;
			unset($group);
		}
		
		//获取分组
		$Grouplist = $Group->order('sort desc')->where('type=2 and status=1')->getField('id,title,createTime');
		foreach ($Grouplist as $gk => $gv) {
			$Grouplist[$gk]['count'] = $Blogs->where('status=1 and type=2 and groupId='.$gk)->count();
		}
	...
	public function insert(){
        ...
        if ($list !== false) { //保存成功
			...
			$this->success('新增成功!',U('/Share/'.$list));
			//$this->show('新增成功!');
        } else {
            //失败提示
            $this->error('新增失败!');
	...
	public function add(){
		...
		$Group = M('Group');
		//获取分组
		$Grouplist = $Group->order('sort desc')->where('type=2')->getField('id,title,createTime');
	...
	function edit() {
		...
		$Group = M('Group');
		//分组列表
		$Grouplist = $Group->order('sort desc')->where('type=1')->getField('id,title,createTime');
		$this->assign('Grouplist',$Grouplist);// 分组输出
		//dump($Grouplist);
		//exit;
	...
	public function review(){
		...
		//对二级评论循环输出
			$review = $Comment->order('createTime desc')->where('status=1 and pid='.$sv['id'].' and fromId='.I('record'))->select();
			if(is_array($review)){
    ...
    public function read(){
		...
		if(empty($id)) $id=$I[2];
		$blog = $Blogs->where('status=1 and type=2 and id='.$id)->select();
		$blog = $blog['0'];
		...
		//加载分组名
		$group = $Group->where('type=2 and id='.$blog['groupId'])->getField('title');
		$blog['group'] = (empty($group)) ? '未知分组' : $group;
		
		/*
		//加载评论
		$Comment = D('Comment');
		$count = $Comment->where('status=1 and pid=0 and type=2 and fromId='.$id)->count();// 查询满足要求的总记录数
		import('ORG.Util.Page', LIB_PATH);
		...
		*/
		...
		//获取上下篇
		$prev = $Blogs->where('id<'.$blog['id'].' and status=1 and type=2')->find();//获取上一篇
		$next = $Blogs->where('id>'.$blog['id'].' and status=1 and type=2')->find();//获取下一片
