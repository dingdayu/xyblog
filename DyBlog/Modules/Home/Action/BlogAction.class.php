...
class BlogAction extends CommonAction {
	...
	public function index(){
		...
		$blogcount1      = $Blogs->where('status=1 and type=1')->count();// 查询满足要求的总记录数
		...
		$this->assign('blogcount',$blogcount);// 记录总数
    ...
    public function read(){
		...
		if(empty($id)) $id=$I[2];
		$blog = $Blogs->where('status=1 and type=1 and id='.$id)->select();
		$blog = $blog['0'];
		...
		//加载分组名
		$group = $Group->where('status=1 and type=1 and id='.$blog['groupId'])->getField('title');
		...
		//加载评论
		$Comment = D('Comment');
		$count      = $Comment->where('status=1 and pid=0 and type=1 and fromId='.$id)->count();// 查询满足要求的总记录数
		import('ORG.Util.Page', LIB_PATH);
		...
		*/
		...
		$prev = $Blogs->where('id<'.$blog['id'].' and status=1 and type=1')->find();//获取上一篇
		$next = $Blogs->where('id>'.$blog['id'].' and status=1 and type=1')->find();//获取下一片
