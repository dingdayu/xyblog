...
class WeChatOAuth{
    ...
    //便捷的获取用户资料
    public static function getQuickUserInfo($scope='snsapi_base',$state=1 ){
    	...
    	if(!$code && !$_SESSION['openid']){
    		self::getCode($url, $state=1, $scope=$scope);
    	}
    	if(!$_SESSION['openid'] && $code){
			$ret = self::getAccessTokenAndOpenId($code);

			if(empty($ret['openid'])){
				return $ret['errmsg'];
			}
			$_SESSION['openid'] = $ret['openid']; //openid

            if($scope == "snsapi_userinfo"){
                //获取用户资料
                $userinfo = self::getUserInfo($ret['access_token'],$ret['openid']);
                $_SESSION['userinfo'] = $userinfo; //openid
                if (!empty($userinfo)) {
                    return array_merge($userinfo,$ret);
                }
	...
	public static function get_page_url(){
		...
		if ($_SERVER["SERVER_PORT"] != "80") { 
			$pageURL .= $_SERVER["SERVER_NAME"].":".$_SERVER["SERVER_PORT"].strip_tags($_SERVER["REQUEST_URI"]); 
		} else { 
			$pageURL .= $_SERVER["SERVER_NAME"].strip_tags($_SERVER["REQUEST_URI"]); 
		} 
		return $pageURL; 
	}
